defmodule GuessTheSong.Quiz do
  ...
  def run_round(guild_id, period) do
    {discord_id, lastfm, index} = Server.get_random_track(guild_id)
    {:ok, lastfm_track} = Api.Lastfm.fetch_top_track_from_index(lastfm.name, period, index)
  ...
  """
  @spec run_quiz(String.t(), String.t(), integer(), String.t()) :: :ok
  def run_quiz(guild_id, text_channel_id, rounds, period) do
    Enum.each(1..rounds, fn _ ->
      case run_round(guild_id, period) do
        {:ok, track} ->
          :timer.sleep(1000)
